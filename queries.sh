...
echo "$($PSQL "SELECT name FROM teams AS t RIGHT JOIN games AS g ON t.team_id = g.winner_id OR t.team_id = g.opponent_id WHERE round='Eighth-Final' AND year=2014 ORDER BY name")"
...
echo "$($PSQL "SELECT year, name FROM teams AS t INNER JOIN games g ON t.team_id = g.winner_id WHERE round='Final' ORDER BY year")"
